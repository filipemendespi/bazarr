...
class PlexSyncStatusService:
            ...
            for lib in libraries:
                ...
                if lib.type == 'movie':
                    ...
                    movie_stats = database.execute(
                        select(func.count(TablePlexMovies.id).label('count'),
                    ...
                    show_stats = database.execute(
                        select(func.count(TablePlexShows.id).label('show_count'),
                    ...
                    episode_stats = database.execute(
                        select(func.count(TablePlexEpisodes.id).label('episode_count'),
                              func.max(TablePlexEpisodes.updated_at_timestamp).label('episode_last_update'))
                        .join(TablePlexShows, TablePlexEpisodes.show_plex_id == TablePlexShows.plex_id)
                        .where(TablePlexShows.library_key == lib.key)
                    ).first()
    ...
    def get_content_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # Movie statistics
            movie_stats = database.execute(
                select(func.count(TablePlexMovies.id).label('total_movies'),
                      func.count(func.distinct(TablePlexMovies.library_key)).label('movie_libraries'))
            ).first()
            
            # Show statistics
            show_stats = database.execute(
                select(func.count(TablePlexShows.id).label('total_shows'),
                      func.count(func.distinct(TablePlexShows.library_key)).label('show_libraries'))
            ).first()
            
            # Episode statistics
            episode_stats = database.execute(
                select(func.count(TablePlexEpisodes.id).label('total_episodes'))
            ).first()
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stale_libraries = database.execute(
                select(func.count(TablePlexLibraries.id))
                .where(
